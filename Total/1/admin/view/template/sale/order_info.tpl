  ...
  <div class="box">
    ...
    <div class="content">
      <div class="vtabs"><a href="#tab-order"><?php echo $tab_order; ?></a><a href="#tab-payment"><?php echo $tab_payment; ?></a>
        ...
        <a href="#tab-reservi"> Datos Reservacion </a><a href="#tab-history"><?php echo $tab_history; ?></a>
        <?php if ($maxmind_id) { ?>
      ...
      <div id="tab-reservi" class="vtabs-content">
        <?php foreach ($products as $product){
          //echo'<pre>'; print_r($product); echo'</pre>';
          $myquery='select * from ord_resrv where idorder='.$order_id.' and idpro='.$product['product_id'];
          $dtagen=$this->db->query($myquery);
          $myquery='select * from personas where idorder='.$order_id.' and idpro='.$product['product_id'];
          $dtaper=$this->db->query($myquery); $cnt=1;
          $myquery='select * from ord_habs where idorder='.$order_id.' and idpro='.$product['product_id'];
          $dtahab=$this->db->query($myquery); $cnt2=1; ?>
          <table class="form">
            <tr>
              <td colspan="3">
                Producto: <a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a>
                <?php $namepdf=$this->db->query('select * from orders_pdf where idorder='.$order_id); 
                if(isset($namepdf->rows[0]['nampdf'])){ $pdffilename=$namepdf->rows[0]['nampdf']; ?>
                  <br />Documento de <a href="../orders_pdf/<?php echo $pdffilename;?>">reservaci&oacute;n en PDF</a>
                <?php }else{ echo '<br />(No hay datos del documento PDF)'; } ?>
              </td>
            </tr>
    ...
    <!--Ciudad de Salida-->
            ...
            <tr>
              <td colspan="2">Saliendo desde ciudad </td>
              <td> <?php echo (isset($dtagen->rows[0]['cddsal'])?$dtagen->rows[0]['cddsal']:'No hay datos');?></td>
            </tr>
    ...
    <!--Fecha-->
            ...
            <tr>
              ...
              <td>  <?php echo (isset($dtagen->rows[0]['finicio'])?cutTimeFDate($dtagen->rows[0]['finicio']):'no hay datos');
                echo (isset($dtagen->rows[0]['ffin'])?' hasta: '.cutTimeFDate($dtagen->rows[0]['ffin']):'');
              ?> </td>
            </tr>
      ...
      <!--Personas-->      
            <tr>
             <td colspan="3">Personas incluidas en la reservacion (si es hotel solo se muestra la persona a cargo del grupo)</td> </tr>

            <?php foreach($dtaper->rows as $persona){ ?>
              <tr> <td rowspan="5">Persona <?php echo $cnt++;?></td> <td>Nombre: </td><td><?php echo $persona['nombre']; ?>
                
              </td> </tr>
              <tr> <td>Direccion: </td><td><?php echo $persona['direccion']; ?></td> </tr>
              <tr> <td>Telefono: </td><td><?php echo $persona['telefono']; ?></td> </tr>
              <tr> <td>Email: </td><td><?php echo $persona['email']; ?></td> </tr>
              <tr> <td>Edad: </td><td><?php echo ($persona['adulto']?'Adulto':'Menor'); ?></td> </tr>
...
?>
          ...
          <!--Habitacioones Reservada-->

            <tr> <td colspan="3">Habitaciones reservadas</td> </tr>
            
            <?php foreach($dtahab->rows as $habitacion){ ?>
              <tr> <td rowspan="4">Habitacion <?php echo $cnt2++;?></td> <td>Tipo: </td><td><?php echo $habitacion['tipohab']; ?></td> </tr>
              <tr> <td>Categoria: </td><td><?php echo $habitacion['catg']; ?></td> </tr>
              <tr> <td>Costo: </td><td><?php echo $habitacion['costo']; ?></td> </tr>
              <tr> <td>Cantidad: </td><td><?php echo $habitacion['quantity']; ?></td> </tr>
