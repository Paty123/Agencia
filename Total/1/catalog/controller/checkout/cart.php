...
class ControllerCheckoutCart extends Controller {
	...
	public function add(){
                $wdata=$this->request->post;
		$this->language->load('checkout/cart');
		$json = array();
		if (isset($wdata['product_id'])){ $product_id = $wdata['product_id']; }
                else{ $product_id = 0; }
		$this->load->model('catalog/product');
		$product_info=$this->model_catalog_product->getProduct($product_id);
		if($product_info){
			if (isset($wdata['quantity'])){ $quantity = $wdata['quantity']; }
                        else{ $quantity = 1; }
			if (isset($wdata['option'])){ $option = array_filter($wdata['option']); }
                        else{ $option = array(); }
			$product_options=$this->model_catalog_product->getProductOptions($wdata['product_id']);
			...
			if (!$json) {
				$this->cart->add($wdata['product_id'],$quantity,$option,$wdata);
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $wdata['product_id']), $product_info['name'], $this->url->link('checkout/cart'));
				unset($this->session->data['shipping_method']);
				unset($this->session->data['shipping_methods']);
				unset($this->session->data['payment_method']);
				unset($this->session->data['payment_methods']);
				...
				$json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/product', 'product_id=' . $wdata['product_id']));
			}
		}
